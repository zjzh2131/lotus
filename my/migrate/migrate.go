...
func MigrateC1Out(p *MigrateParam) error {
	f := &FtpClient{
		ID:       "",
...
func MigrateC2Out(p *MigrateParam) error {
	f := &FtpClient{
		ID:       "",
		Port:     p.FtpEnv.FtpPort,
		User:     p.FtpEnv.FtpUser,
	...
	srcFolder := filepath.Join(p.FromPath, "c2Out", p.SectorID)
	srcPath := filepath.Join(srcFolder, "c2Out")
	existed, err := Exists(srcPath)
	if err != nil {
		return xerrors.New("check file failed:" + srcPath)
	}
	...
	desPath := filepath.Join(p.StorePath, "c2Out", p.SectorID, "c2Out")
	err = f.Mkdir_P(desPath)
	if err != nil {
		return xerrors.New(desPath + " Mkdir_P err " + err.Error())
	}
